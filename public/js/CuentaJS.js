$(document).ready(function(){
	...
	DataTableCuenta.DataTable_Base_Cliente_Lista();
	...
	$('#cbo_empresa_letxsit').multiselect({
		buttonWidth: '100%',
		nonSelectedText: 'Empresa',
		allSelectedText: 'Todo Seleccionado',
		includeSelectAllOption: true,
		onChange: function(element, checked) {
			var brands = $('#cbo_empresa_letxsit option:selected');
			var selected = [];
			$(brands).each(function(index, brand){
				selected.push([$(this).val()]);
			});
			...
			//console.log(selected);
			$("#hdcbo_empresa_letxsit").val(selected);
		},
	...
	$('#btn_empresa_letxsit-deselectAll-all').on('click', function() {
		$('#cbo_empresa_letxsit').multiselect('deselectAll', true);
		$('#cbo_empresa_letxsit').multiselect('updateButtonText');
	});
	...
	// CON querySelector ES NECESARIO VALIDAR SI EXISTE EL ELEMENTO SINO NO DEJA TRABAJAR ESTE JS CON OTRAS PAGINAS WEBS AL REUTILIZARLO
	var selectionini = document.querySelector('#txtfchemiini') !== null;
	var selectionfin = document.querySelector('#txtfchemifin') !== null;
	...
	$("#form_letsit").on("submit",function(e){
		if($("#hdcbo_empresa_letxsit").val()!=""){
			
			var empresa = $("#hdcbo_empresa_letxsit").val();
			var tabla = 'TMP_LET_X_SIT_'+hoyFecha().toString();

			$("#Pgbar_letsit").modal({backdrop: 'static', keyboard: false}); // Mostrando el Modal de ProgressBar
			$('#Pgbar_letsit .close').css('display', 'none'); // Ocultando el boton Close				

			$("#tmp_name_table_letsit").val(tabla);

			var formato = $("#extensiondwonload").text();



			CuentaAJAX.Exportar_LetraXSituacion(empresa,tabla,function(obj){
				if (obj.rst) {
					if(obj.cant == 0){
						$('#Pgbar_letsit').modal('hide');									
						$('#Aviso_info_cero').modal('toggle');				
					}else{
						$('#total_letsit').text(obj.cant);
						$("#Pgbar_Subtit_letsit").text("Procesando...");
						CuentaAJAX.ProgressBar_LetxSituacion_xlsx(1, false, tabla);
					}					
				}
			});	

		}else{
			$("#vempre_letxsitu").modal();
		}

	});

	$("#Pgbar_letsit").on("hidden.bs.modal", function () {
		...
		$('#total_letsit').text(0);
		$('#done_letsit').text(0);
		$('#Pgbar_Subtit_letsit').text("Explorando información");
		$(".execute-time").text("0 segundos");
