var CuentaAJAX={	
	...
	// }
	Exportar_LetraXSituacion : function(vempresa,vtabla,f_success){
		$.ajax({
			url: '../Index/Exportar_LetraXSituacion',
			type:'POST',
			dataType:'json',
			data:{
				empresa : vempresa,
	...
	ProgressBar_LetxSituacion_xlsx : function(offset, batch = 1, vtabla) {

		batch = 1;

		$.ajax({
			url: '../Index/ProgressBar_LetxSituacion_xlsx',
			type:'POST',
			dataType:'json',
			data:{
				offset: offset,
			...
			success:function(response){
				...
				$('#done_letsit').text(response.executed);
				$('.execute-time').text(response.execute_time);

				//insert text in textarea
				$("#Rpt_letsit").append('EJECUTADOS: '+response.executed+' \n');
				$("#Rpt_letsit").append('PORCENTAJE: '+response.percentage+'% \n');
				$("#Rpt_letsit").append('TOTAL: '+$("#total_letsit").val()+' \n');
				$("#Rpt_letsit").append('TIEMPO DE EJECUCI&Oacute;N: '+response.execute_time+' \n');
				$("#Rpt_letsit").append('----------------------------------------------------------------------------------------------------------------------------------------------------- \n');
				
				if (response.percentage == 100) {

					$('#Pgbar_letsit .close').css('display', ''); // hibilitando close del modal
					$("#Pgbar_Subtit_letsit").text("Descarga el archivo"); // cambiando titulo del modal
					$('.end-process').show();

					var base_url = window.location.origin;

					$("#Down_letsit").html('Proceso completado.&nbsp;&nbsp;&nbsp;<button onclick="window.location.href=\''+base_url+'/SHEET/documents/files/excel/'+vtabla+'.xlsx\'" type="button" class="btn sample btn-lg btn-sm btn-excel-xlsx" ><i class="fa fa-file-excel"></i><span class="link-title">&nbsp;.xlsx</span></button>');
					
					CuentaAJAX.DeleteTableTMPADM(vtabla);
				} else {
					var newOffset = offset + batch;					
					CuentaAJAX.ProgressBar_LetxSituacion_xlsx(newOffset, batch, vtabla);
				}
			},
			error:function(XMLHttpRequest, textStatus, errorThrown){
				if (textStatus == 'parsererror') {
	...
	DeleteTableTMPADM: function(vtbl) {
		$.ajax({
			url: '../Index/DeleteTableTMPADM',
			type:'POST',
			dataType:'json',
			data:{
				tbl : vtbl
