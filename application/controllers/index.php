...
class Index extends CI_Controller {
	...
	public function index() {
        ...
        $this->layout->setTitle("Repositorio de Documentos");
		...
		// // FullScreenSlideShow
		$this->layout->css(array(base_url()."public/Librerias/FullScreenSlideShow/css/demo.css"));
		$this->layout->css(array(base_url()."public/Librerias/FullScreenSlideShow/css/style1.css"));
		$this->layout->js(array(base_url()."public/Librerias/FullScreenSlideShow/js/modernizr.custom.86080.js"));

		$this->layout->js(array(base_url()."public/js/LoginAJAX.js"));
		$this->layout->js(array(base_url()."public/js/LoginJS.js"));

		$this->layout->js(array(base_url()."public/Librerias/TripleClick/tripleclick.js"));
	...
	public function historico_detalle_cuenta(){
		...
		$this->layout->css(array(base_url()."public/Librerias/DataTable/css/dataTables.jqueryui.min.css"));
		$this->layout->css(array(base_url()."public/Librerias/DataTable/css/buttons.jqueryui.min.css"));

		$this->layout->js(array(base_url()."public/Librerias/DataTable/js/jquery.dataTables.min.js"));
		$this->layout->js(array(base_url()."public/Librerias/DataTable/js/dataTables.jqueryui.min.js"));
		$this->layout->js(array(base_url()."public/Librerias/DataTable/js/dataTables.buttons.min.js"));
		$this->layout->js(array(base_url()."public/Librerias/DataTable/js/buttons.jqueryui.min.js"));
		$this->layout->js(array(base_url()."public/Librerias/DataTable/js/buttons.html5.min.js"));
		$this->layout->js(array(base_url()."public/Librerias/DataTable/js/buttons.print.min.js"));
		$this->layout->js(array(base_url()."public/Librerias/DataTable/js/buttons.colVis.min.js"));
	...
	public function DeleteTableTMPADM(){
		$this->Cuenta_model->DeleteTableTMPADM($_POST['tbl']);
	...
	public function letras_x_situacion(){
		...
		$this->layout->js(array(base_url()."public/Librerias/JQuery-3.3.1/jquery-3.3.1.js"));
...
// 		$this->layout->js(array("https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.22.2/moment.min.js")); */
		...
		$this->layout->js(array(base_url()."public/js/CuentaJS.js"));
		$this->layout->js(array(base_url()."public/js/CuentaAJAX.js"));
		$this->layout->js(array(base_url()."public/js/DataTableCuenta.js"));


		$this->layout->view('letras_x_situacion');

	}

	public function Exportar_LetraXSituacion(){
		//$vop = 2;
		$vcod = $_POST['empresa'];
		$vtbl = $_POST['tabla'];

		$CantLetSit = $this->Cuenta_model->Contar_Registros_LetSit(2, $vcod, $vtbl);
		$Cantidad = $CantLetSit[0]['CANTIDAD'];

		if($Cantidad > 0) {

			$updProcessDetcu = " UPDATE PROGRESSBAR SET EXECUTED=0,TOTAL=$Cantidad, PERCENTAGE=0,EXECUTE_TIME='',DATE_ADD= GETDATE(),DATE_UPD= GETDATE() WHERE ID_PROCESS = 2 ";
			$ProcessDetcu = $this->mssql->query($updProcessDetcu);

			//$this->Cuenta_model->Listar_Registros_LetSit(1, $vcod, $vtbl, $CantLetSit[0]['CANTIDAD'] );

			$RutaRaiz = FCPATH;
			$output = Shell_Exec('powershell -InputFormat none -ExecutionPolicy ByPass -NoProfile -Command "'.$RutaRaiz.'documents\tareas\exportar_LetSit_plaintext.ps1 -OP 1   -COD \''.$vcod.'\' -RUTARAIZ '.$RutaRaiz.' -FILENAME '.$vtbl.' "  ');

			$mystring = $output;
			$findme   = 'filas copiadas';
			$pos = strpos($mystring, $findme);

			echo json_encode(array('rst' => true, 'msg' => 'Se Proceso Exitosamente', 'cant'=>$Cantidad ));
	...
	public function ProgressBar_LetxSituacion_xlsx(){
		$RutaRaiz = FCPATH;
		...
		// foreach ($recorrer->result() as $rows) {
			...
			// $xls->getActiveSheet()->setCellValueExplicit($columna[$col+0].$fila, $rows->COD,PHPExcel_Cell_DataType::TYPE_STRING)->getStyle($columna[$col+0].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+1].$fila, $rows->EMPRESA)->getStyle($columna[$col+1].$fila)->applyFromArray($font); 
			// $xls->getActiveSheet()->SetCellValue($columna[$col+2].$fila, $rows->COD_VEND)->getStyle($columna[$col+2].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); // 2
			// $xls->getActiveSheet()->SetCellValue($columna[$col+3].$fila, $rows->VEND)->getStyle($columna[$col+3].$fila)->applyFromArray($font);			
			// $xls->getActiveSheet()->setCellValueExplicit($columna[$col+4].$fila, $rows->NRO_LETRA,PHPExcel_Cell_DataType::TYPE_STRING)->getStyle($columna[$col+4].$fila)->applyFromArray($font);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+5].$fila, $rows->FEC_EMIS)->getStyle($columna[$col+5].$fila)->applyFromArray($font);
			// $xls->getActiveSheet()->setCellValueExplicit($columna[$col+6].$fila, $rows->COD_CLIE,PHPExcel_Cell_DataType::TYPE_STRING)->getStyle($columna[$col+6].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+7].$fila, $rows->CLIENTE)->getStyle($columna[$col+7].$fila)->applyFromArray($font);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+8].$fila, $rows->PLAZO)->getStyle($columna[$col+8].$fila)->applyFromArray($font);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+9].$fila, $rows->FEC_VENC)->getStyle($columna[$col+9].$fila)->applyFromArray($font);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+10].$fila, $rows->MONEDA)->getStyle($columna[$col+10].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+13].$fila, $rows->IMPORTE_MN)->getStyle($columna[$col+13].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+14].$fila, $rows->IMPORTE_US)->getStyle($columna[$col+14].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+15].$fila, $rows->ESTADO)->getStyle($columna[$col+15].$fila)->applyFromArray($font);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+16].$fila, $rows->DOC_REFER)->getStyle($columna[$col+16].$fila)->applyFromArray($font);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+17].$fila, $rows->OBS)->getStyle($columna[$col+17].$fila)->applyFromArray($font);
			// $xls->getActiveSheet()->SetCellValue($columna[$col+19].$fila, $rows->TRANSACCION)->getStyle($columna[$col+19].$fila)->applyFromArray($font);

			// $updAvanProcessDetcu = " UPDATE $vtabla SET AVANCE= 1 WHERE ID = $voffset ";
			// $AvanProcessDetcu = $this->mssql->query($updAvanProcessDetcu);

			// $updProcessDetcu = " UPDATE PROGRESSBAR SET EXECUTED= EXECUTED + 1, DATE_UPD= GETDATE() WHERE ID_PROCESS = 2 ";
			// $ProcessDetcu = $this->mssql->query($updProcessDetcu);
		
		// }

		//$archivo = file($RutaRaiz.'documents\files\txt\\'.$vtabla.'.txt');

		$file = new SplFileObject($RutaRaiz.'documents\files\txt\\'.$vtabla.'.txt');
		$file->seek($fila); // La lectura del archivo inicia desde 0 hasta su ultima fila o registro
		$array = preg_split("/[\t]/", $file->current());
		...
		foreach($array as $valor) {			
			...
			if($coli == 0){ $xls->getActiveSheet()->setCellValueExplicit($columna[$col+0].$fila, $valor ,PHPExcel_Cell_DataType::TYPE_STRING)->getStyle($columna[$col+0].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); }
			if($coli == 1){ $xls->getActiveSheet()->SetCellValue($columna[$col+1].$fila, $valor)->getStyle($columna[$col+1].$fila)->applyFromArray($font); } 
			if($coli == 2){ $xls->getActiveSheet()->SetCellValue($columna[$col+2].$fila, $valor)->getStyle($columna[$col+2].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); }
			if($coli == 3){ $xls->getActiveSheet()->SetCellValue($columna[$col+3].$fila, $valor)->getStyle($columna[$col+3].$fila)->applyFromArray($font); }			
			if($coli == 4){ $xls->getActiveSheet()->setCellValueExplicit($columna[$col+4].$fila, $valor,PHPExcel_Cell_DataType::TYPE_STRING)->getStyle($columna[$col+4].$fila)->applyFromArray($font); }
			if($coli == 5){ $xls->getActiveSheet()->SetCellValue($columna[$col+5].$fila, $valor)->getStyle($columna[$col+5].$fila)->applyFromArray($font); }
			if($coli == 6){ $xls->getActiveSheet()->setCellValueExplicit($columna[$col+6].$fila, $valor,PHPExcel_Cell_DataType::TYPE_STRING)->getStyle($columna[$col+6].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); }
			if($coli == 7){ $xls->getActiveSheet()->SetCellValue($columna[$col+7].$fila, $valor)->getStyle($columna[$col+7].$fila)->applyFromArray($font); }
			if($coli == 8){ $xls->getActiveSheet()->SetCellValue($columna[$col+8].$fila, $valor)->getStyle($columna[$col+8].$fila)->applyFromArray($font); }
			if($coli == 9){ $xls->getActiveSheet()->SetCellValue($columna[$col+9].$fila, $valor)->getStyle($columna[$col+9].$fila)->applyFromArray($font); }
			if($coli == 10){ $xls->getActiveSheet()->SetCellValue($columna[$col+10].$fila, $valor)->getStyle($columna[$col+10].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); }
			if($coli == 11){ $xls->getActiveSheet()->SetCellValue($columna[$col+11].$fila, $valor)->getStyle($columna[$col+11].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); }
			if($coli == 12){ $xls->getActiveSheet()->SetCellValue($columna[$col+12].$fila, $valor)->getStyle($columna[$col+12].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); }
			if($coli == 13){ $xls->getActiveSheet()->SetCellValue($columna[$col+13].$fila, $valor)->getStyle($columna[$col+13].$fila)->applyFromArray($font); }
			if($coli == 14){ $xls->getActiveSheet()->SetCellValue($columna[$col+14].$fila, $valor)->getStyle($columna[$col+14].$fila)->applyFromArray($font); }
			if($coli == 15){ $xls->getActiveSheet()->SetCellValue($columna[$col+15].$fila, $valor)->getStyle($columna[$col+15].$fila)->applyFromArray($font); }
			if($coli == 16){ $xls->getActiveSheet()->SetCellValue($columna[$col+16].$fila, $valor)->getStyle($columna[$col+16].$fila)->applyFromArray($font); }
		...
		//$xls->getActiveSheet()->setCellValueExplicit($columna[$col+0].$fila, $array[0] ,PHPExcel_Cell_DataType::TYPE_STRING)->getStyle($columna[$col+0].$fila)->applyFromArray($font)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		...
		$sqlProgress = " SELECT EXECUTED,TOTAL, DATE_ADD,DATE_UPD FROM PROGRESSBAR WHERE ID_PROCESS = 2 ";
		$rslProgress = $this->mssql->query($sqlProgress);
		$arrProgressDetcu = $rslProgress->result_array();
		...
		$updProcessDetcu2 = " UPDATE PROGRESSBAR SET DATE_UPD= GETDATE(),PERCENTAGE= ".$percentage.", EXECUTE_TIME = '".(string)$execute_time."' WHERE ID_PROCESS = 2 ";
		$ProcessDetcu2 = $this->mssql->query($updProcessDetcu2);
...
// 		$this->layout->js(array("https://cdnjs.cloudflare.com/ajax/libs/moment.js/2.22.2/moment.min.js")); */
		...
		// DataTables
		//$this->layout->css(array(base_url()."public/Librerias/DataTables/datatables.css"));
		//$this->layout->js(array(base_url()."public/Librerias/DataTables/datatables.js"));


		// boots trap
		// $this->layout->js(array("https://cdn.datatables.net/1.10.20/js/jquery.dataTables.min.js"));		
		// $this->layout->css(array("https://cdn.datatables.net/1.10.20/css/dataTables.bootstrap.min.css"));
		// $this->layout->js(array("https://cdn.datatables.net/1.10.20/js/dataTables.bootstrap.min.js"));


		// jquery ui

		// $this->layout->css(array("https://cdn.datatables.net/1.10.20/css/dataTables.jqueryui.min.css"));
		// $this->layout->css(array("https://cdn.datatables.net/buttons/1.6.0/css/buttons.jqueryui.min.css"));

		// $this->layout->js(array("https://cdn.datatables.net/1.10.20/js/jquery.dataTables.min.js"));
		// $this->layout->js(array("https://cdn.datatables.net/1.10.20/js/dataTables.jqueryui.min.js"));
		// $this->layout->js(array("https://cdn.datatables.net/buttons/1.6.0/js/dataTables.buttons.min.js"));
		// $this->layout->js(array("https://cdn.datatables.net/buttons/1.6.0/js/buttons.jqueryui.min.js"));
		// $this->layout->js(array("https://cdn.datatables.net/buttons/1.6.0/js/buttons.html5.min.js"));
		// $this->layout->js(array("https://cdn.datatables.net/buttons/1.6.0/js/buttons.print.min.js"));
		// $this->layout->js(array("https://cdn.datatables.net/buttons/1.6.0/js/buttons.colVis.min.js"));
		// DataTable/css/
	...
	public function ListCliente(){
		...
		$this->Cuenta_model->ListCliente($draw,$row,$rowperpage,$columnIndex,$columnName,$columnSortOrder,$searchValue);
